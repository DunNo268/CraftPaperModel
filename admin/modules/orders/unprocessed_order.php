<?php 
    ...
    $sql_order = "SELECT * FROM orders WHERE status_id = '0';";
    ...
    $sql_pagination = "SELECT * FROM orders WHERE status_id = '0' ORDER BY order_id ASC LIMIT $start,$rowPerPage";
...
<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">			
	...
	<div class="row">
		<div class="col-lg-12">
			<h1 class="page-header">Đơn chưa xử lý</h1>
		</div>
	</div><!--/.row-->
                        ...
                        <tbody>
                            ...
                            ?> 
                                <tr>
                                    ...
                                    <td><span class="label label-danger">Chưa xử lý</span></td>
                                    <td class="form-group">
                                        <?php if (checkPrivilege('index.php?page=order_detail1&order_id='.$row['order_id'])) { ?>
                                            <a title="Xem" href="index.php?page=order_detail1&order_id=<?php echo $row['order_id']; ?>" class="btn btn-primary">
                                                <i class="glyphicon glyphicon-eye-open"></i>
                                            </a>
                                        <?php } ?>
                                            <a title="Cập nhật trạng thái" onclick="return confirmUpd();" href="modules/orders/edit_order1.php?order_id=<?php echo $row['order_id']; ?>" class="btn btn-warning">
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <?php if($current_page > 1) { ?>
                                <li class="page-item"><a class="page-link" href="index.php?page=order_unprocessed&current_page=<?php echo $current_page-1; ?>">&laquo;</a></li>
                            <?php }else { ?>
                            ...
                            <?php for($i = 1; $i <= $totalPage; $i++) { 
                                    if($i > $current_page - 3 && $i < $current_page + 3) { 
                                        if($i == $current_page) {   
                            ?>
                                            <li class="page-item active"><a class="page-link" href="index.php?page=order_unprocessed&current_page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                                        <?php }else { ?>
                                            <li class="page-item"><a class="page-link" href="index.php?page=order_unprocessed&current_page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php 
                                    }
                                }
                            }
                            ...
                            <?php if($current_page < $totalPage) { ?>
                                <li class="page-item"><a class="page-link" href="index.php?page=order_unprocessed&current_page=<?php echo $current_page + 1; ?>">&raquo;</a></li>
                            <?php }else {?>
...
<script>
    ...
    function confirmUpd() {
        return confirm("Bạn có chắc chắn muốn chuyển đơn này sang trạng thái đang xử lý?");
    }
</script>	
